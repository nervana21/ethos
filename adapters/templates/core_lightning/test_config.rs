//! Test configuration for Core Lightning RPC testing
//!
//! This module provides configuration utilities for running Core Lightning nodes in test environments.
...
/// TestConfig represents the configuration needed to run a Core Lightning node in a test environment.
...
pub struct TestConfig {
    /// The port number for RPC communication with the Core Lightning node.
    /// A value of 0 indicates that an available port should be automatically selected.
    pub rpc_port: u16,
    /// The username for RPC authentication.
    /// Can be customized to match your `lightning.conf` `rpcuser` setting.
    pub rpc_username: String,
    /// The password for RPC authentication.
    /// Can be customized to match your `lightning.conf` `rpcpassword` setting.
    pub rpc_password: String,
    /// Which Bitcoin network to run against.
    pub network: Network,
    /// Extra command-line arguments to pass to lightningd
