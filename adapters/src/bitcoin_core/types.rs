...
///
/// let by_hash = HashOrHeight::Hash("000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f".to_string());
...
impl HashOrHeight {
	...
	pub fn from_json(value: &serde_json::Value) -> Option<Self> {
		value
			.as_str()
			.map(|hash| HashOrHeight::Hash(hash.to_string()))
	...
	/// use ethos_adapters::bitcoin_core::types::HashOrHeight;
	///
	/// let hash = HashOrHeight::Hash("abc123".to_string());
	/// let json = hash.to_json();
	/// assert_eq!(json.as_str(), Some("abc123"));
	/// ```
	pub fn to_json(&self) -> serde_json::Value {
		match self {
			HashOrHeight::Hash(hash) => serde_json::Value::String(hash.clone()),
...
mod tests {
	...
	#[test]
	fn test_core_adapter_emits_custom_type() {
		// Load the IR file directly (IR-based architecture)
		let ir_path = Path::new("../resources/ir/bitcoin.ir.json");
		let protocol_ir = ProtocolIR::from_file(ir_path).expect("Failed to load IR file");
		let mut found_custom_type = false;
		for module in protocol_ir.modules() {
			for def in module.definitions() {
				if let ProtocolDef::RpcMethod(rpc_def) = def {
					for param in &rpc_def.params {
						if param.param_type.name == "HashOrHeight" {
							assert_eq!(param.param_type.kind, TypeKind::Custom);
							assert_eq!(
								param.param_type.base_type,
								Some("HashOrHeight".to_string())
							);
							found_custom_type = true;
						}
					}
					if let Some(result) = &rpc_def.result {
						if result.name == "HashOrHeight" {
							assert_eq!(result.kind, TypeKind::Custom);
							assert_eq!(result.base_type, Some("HashOrHeight".to_string()));
							found_custom_type = true;
						}
					}
				}
		...
		assert!(found_custom_type, "No HashOrHeight Custom types found in the protocol IR");
	}

	#[test]
	fn test_ir_contains_expected_structure() {
		let ir_path = Path::new("../resources/ir/bitcoin.ir.json");
		let protocol_ir = ProtocolIR::from_file(ir_path).expect("Failed to load IR file");
		assert!(!protocol_ir.modules().is_empty(), "Protocol IR should have at least one module");
		let mut rpc_method_count = 0;
		for module in protocol_ir.modules() {
			for def in module.definitions() {
				if let ProtocolDef::RpcMethod(_) = def {
					rpc_method_count += 1;
				}
			}
		}
		assert!(rpc_method_count > 0, "Should have at least one RPC method");
	}

	#[test]
	fn test_custom_union_serialization() {
		let custom = ir::TypeDef {
			name: "HashOrHeight".into(),
			description: "Core-specific disjunction".into(),
			kind: TypeKind::Custom,
			fields: None,
			variants: None,
			base_type: Some("HashOrHeight".into()),
			canonical_name: None,
			protocol_type: None,
		};
		let json =
			serde_json::to_string_pretty(&vec![custom]).expect("Failed to serialize custom type");
