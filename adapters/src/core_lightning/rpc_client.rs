...
impl crate::rpc_adapter::BackendProvider for CoreLightningRpcClient {
	fn implementation() -> types::Implementation {
		types::Implementation::CoreLightning
	}
...
impl CoreLightningRpcClient {
	...
	pub fn from_env() -> Result<Self, RpcClientError> {
	...
	async fn try_call_http(
		&self, client: &reqwest::Client, request_body: &Value,
	...
	async fn try_call_unix_socket(
		&self, socket_path: &PathBuf, request_body: &Value,
	) -> Result<Value, RpcClientError> {
