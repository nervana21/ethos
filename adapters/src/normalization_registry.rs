...
impl NormalizationRegistry {
	...
	pub fn add_method_mapping(
		&mut self, adapter: AdapterKind, canonical: &str, adapter_specific: &str,
	) {
	...
	fn normalize_value_recursive(
		&self, value: &Value, path: &str, metadata: &mut NormalizationMetadata,
	) -> Value {
		match value {
			Value::Object(map) => {
	...
	fn apply_unit_conversions(
		&self, field_name: &str, value: &Value, metadata: &mut NormalizationMetadata,
	) -> Value {
