...
/// use types::{MethodResult, ParseError};
/// use ir::RpcDef;
///
...
///     
///     fn parse_response_schema(&self, rpc: &RpcDef) -> Result<Option<Vec<MethodResult>>, ParseError> {
...
pub trait TypeAdapter: Send + Sync {
	...
	///
	/// # Returns
	/// * `Ok(Some(Vec<MethodResult>))` - Parsed response schema if available
	/// * `Ok(None)` - If no structured response schema exists
	/// * `Err(ParseError)` - If schema parsing failed due to malformed data
	fn parse_response_schema(
		&self, rpc: &RpcDef,
	) -> Result<Option<Vec<crate::MethodResult>>, crate::errors::ParseError>;
