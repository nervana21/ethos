...
fn create_test_rpc_def(
	results: Vec<MethodResult>, _raw: serde_json::Map<String, serde_json::Value>,
) -> RpcDef {
	RpcDef {
...
fn test_parse_response_schema() {
	...
	assert!(parsed_deep.is_ok());
	let parsed_deep_results = parsed_deep.unwrap();
	assert!(parsed_deep_results.is_some());
	assert_eq!(parsed_deep_results.unwrap().len(), 1);
