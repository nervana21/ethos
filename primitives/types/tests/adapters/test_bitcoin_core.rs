...
fn create_method_result(
	type_: &str, key_name: &str, description: &str, optional: bool,
) -> MethodResult {
	MethodResult {
...
fn test_parse_response_schema() {
	...
	assert!(adapter.parse_response_schema(&method_empty).unwrap().is_none());
	...
	let parsed_results = parsed.unwrap().unwrap();
	...
	assert!(parsed.as_ref().unwrap().is_some());
	let parsed_results = parsed.unwrap();
	assert_eq!(parsed_results.as_ref().unwrap().len(), 1);
	assert_eq!(parsed_results.as_ref().unwrap()[0].key_name, ""); // key_name is empty for top-level results
	assert_eq!(parsed_results.unwrap()[0].inner.len(), 1);
	...
	let parsed_results = parsed.unwrap();
	assert_eq!(parsed_results.as_ref().unwrap().len(), 1);
	assert_eq!(parsed_results.as_ref().unwrap()[0].inner.len(), 2);
	assert_eq!(parsed_results.as_ref().unwrap()[0].inner[0].key_name, "field1");
	assert_eq!(parsed_results.as_ref().unwrap()[0].inner[1].key_name, "field2");
