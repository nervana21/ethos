...
pub enum CodegenError {
	...
	/// Schema parsing/validation error from protocol types.
	#[error(transparent)]
	Schema(#[from] types::SchemaError),
...
fn render_primitive(type_def: &TypeDef) -> String {
	type_def.name.clone()
}
...
impl<'a> CanonicalTypeResolver<'a> {
	...
	pub fn new(registry: &'a TypeAliasRegistry) -> Self {
		Self { registry }
	}
...
pub trait CodeGenerator {
	...
	fn validate(&self, _methods: &[RpcDef]) -> Result<()> {
		Ok(())
	}
}
