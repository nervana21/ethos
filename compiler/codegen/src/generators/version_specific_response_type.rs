...
impl VersionSpecificResponseTypeGenerator {
	...
	pub fn from_ir(
		version: ProtocolVersion, implementation: String, _ir: &ProtocolIR,
	) -> Result<Self> {
	...
	fn generate_ir_field(
		&self, buf: &mut String, field: &ir::FieldDef, _struct_name: &str, _rpc_name: &str,
	) -> Result<()> {
	...
	fn sanitize_field_name(&self, name: &str) -> String {
		crate::utils::sanitize_field_name(name)
	}
	...
	fn generate_primitive_wrapper(
		&self, struct_name: &str, type_name: &str, doc_comment: &Option<String>,
	) -> Result<String> {
		let mut buf = String::new();
	...
	fn collect_nested_types(
		&self, type_name: &str, nested_types: &mut std::collections::HashSet<String>,
	) {
	...
	fn generate_type_alias(&self, type_name: &str) -> Result<String> {
		...
		let rust_type = match type_name {
			"RawTransaction" => "String",            // Hex-encoded transaction
			"ScriptType" => "String",                // Script type as string
			"ActivityEntry" => "String",             // Activity entry as string
			"Bip125Replaceable" => "bool",           // Boolean for replaceable status
			"ScriptPubkey" => "String",              // Hex-encoded script
			"GetWalletInfoScanning" => "String",     // Scanning info as string
			"AddressPurpose" => "String",            // Address purpose as string
			"Bip9Info" => "String",                  // BIP9 info as string
			"TransactionCategory" => "String",       // Transaction category as string
			"SubmitPackageTxResultFees" => "String", // Fees as string
			_ => "String",                           // Default to String for better compatibility
		};
