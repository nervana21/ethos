...
use ir::RpcDef;
use types::{type_adapter::TypeAdapter, Implementation, ProtocolVersion, TypeRegistry};
...
impl VersionSpecificClientTraitGenerator {
	...
	fn render_client_trait(&self, template: &str, methods: &[&RpcDef]) -> String {
		...
		let trait_method_signatures = methods
			...
			.collect::<Vec<_>>()
			.join("\n\n");
		out = out.replace("{{TRAIT_METHOD_SIGNATURES}}", &trait_method_signatures);
		...
		let trait_method_implementations = methods
			...
			.collect::<Vec<_>>()
			.join("\n\n");
		out = out.replace("{{TRAIT_METHOD_IMPLEMENTATIONS}}", &trait_method_implementations);
		out
	}
	...
	fn render_method_signature(&self, rpc: &RpcDef) -> String {
		...
		// Version-specific method documentation removed - IR doesn't track per-type versions

		// Add deprecation warning if method is deprecated in this version
		if self.is_method_deprecated(rpc) {
			writeln!(buf, "    #[deprecated]").expect("Failed to write deprecation attribute");
		}
	...
	/// Check if a method is deprecated in this version
	fn is_method_deprecated(&self, _rpc: &RpcDef) -> bool {
		// This would check the type metadata for deprecation information
