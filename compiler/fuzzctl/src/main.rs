...
fn run_docker_capture(
	fuzz_dir: &Path, service: &str, profile: &str, timeout_secs: Option<u64>, logs_dir: &Path,
...
fn run_docker_detached(
	fuzz_dir: &Path, service: &str, profile: &str, out_dir: &Path,
) -> Result<(), FuzzctlError> {
...
fn stream_logs(
	fuzz_dir: &Path, out_dir: &Path, service: &str, follow: bool,
) -> Result<(), FuzzctlError> {
...
fn build_fuzz_paths(
	fuzz_dir: &Path, out_dir: &Path, service: &str,
...
fn write_feedback(
	out_dir: &Path, duration_seconds: u64, exit_code: i32, corpus_entries: u64, crash_count: u64,
) -> Result<(), FuzzctlError> {
