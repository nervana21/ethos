...
async fn main() {
	...
	if args.iter().any(|a| a == "pipeline") {
		...
		let project_root = match pipeline::path_utils::find_project_root() {
			Ok(root) => root,
			Err(e) => {
...
fn dump_ir_for_implementation(
	implementation: &str, output_path: &std::path::Path,
...
fn compile_with_ir(
	mut ir: ir::ProtocolIR, implementation: Implementation, version: &ProtocolVersion,
	output_dir: &std::path::Path,
) -> Result<(), Box<dyn std::error::Error>> {
	use pipeline::codegen_orchestration::{analyze_implementation, generate_into};
	use pipeline::path_utils::find_project_root;
